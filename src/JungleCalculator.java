...
public class JungleCalculator
{
    ...
    private static int[] extractAndSortArray(int[][] i, int GoE){
        int cnt = 0;
        int [] arr = new int[i.length];
        if (GoE > 1) jcm.print("Oops, cannot calculate amount using this method yet, retry using 0 or 1 as" +
                                                                                                              " value");
        for(int [] y : i)
        {
            arr[cnt] = y[GoE] * creeps[cnt][2];
